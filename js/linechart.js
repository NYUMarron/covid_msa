function lineChart(data, div, type) {
    ...
    let dataTransformedMSA = caseDataPrep(data_msa, '%m/%d/%Y');
    let dataTransformedUSA = caseDataPrep(data_usa, '%m/%d/%Y');
    let dataTransformedMSATotal = caseDataPrep(data_msa_total, '%m/%d/%Y');
    let dataTransformedUSATotal = caseDataPrep(data_usa_total, '%m/%d/%Y');

    const timeEnd = d3.max(dataTransformedMSA.map(d => d.date));
    const timeStart = subtractDays(timeEnd, 90);


    dataTransformedMSA = dataTransformedMSA.filter(d => d.date >= timeStart);
    dataTransformedUSA = dataTransformedUSA.filter(d => d.date >= timeStart);
    dataTransformedMSATotal = dataTransformedMSATotal.filter(d => d.date >= timeStart);
    dataTransformedUSATotal = dataTransformedUSATotal.filter(d => d.date >= timeStart);
...
function caseDataPrep(data, timeparse) {
